...
// Estado inicial con moneda por defecto (Guaraníes)
const [currencyId, setCurrencyId] = useState<string>('cur3'); // cur3 es PYG en tu ejemplo
...
export default function TransactionForm({ initialDirection = 'expense' }: TransactionFormProps) {
  ...
  const [currencyId, setCurrencyId] = useState<string>('cur1'); // Euro por defecto
  ...
  const getSelectedCurrencySymbol = () => {
    const selected = mockCurrencies.find(currency => currency.id === currencyId);
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-6">
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {[10000, 20000, 50000, 100000].map(value => (
              ...
              >
                {getSelectedCurrencySymbol()}{value}
              </button>
            ))}
          </div>
